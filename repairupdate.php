...
$TID=intval($_GET['id']);
$message = '';
$connect = new PDO("mysql:host=sql6.freemysqlhosting.net;dbname=sql6408202", "sql6408202", "lXuT18G82u");
...
foreach ($rms as $row) {
  ...
  foreach($result as $row)
  {
    $output .= '
      <tr>
        <td>'.$row["Name"].'</td>
         <td><img src="yachtphoto/'.$row['Photo'].'" width="250" height="250"></td>
...
<br>
 <button type="submit" name="sub" id="submit" class="btn btn-default">test</button>
<br>
</form>
                            </div>
                            </div>
